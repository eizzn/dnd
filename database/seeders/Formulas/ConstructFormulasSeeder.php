...
<p>On the final day, the listed spells must be cast, at which point, the crafter suffers 1 CON damage.</p>';
        $helper->saveFormula($formula, ['Construct', 'Necromancy'], [
            'skills' => [
                'Crafting' => ['dc' => 12, 'meta' => 'Basketweaving'],
                'Nature'   => ['dc' => 10],
            ],
...
<ul>
    ...
    <li>7 Drops of fresh blood (2 CON damage)</li>
</ul>';
...
<ul>
    ...
    <li>Assembling the body requires a DC 13 Craft (Leatherworking) check and a DC 13 Medicine (Administer First Aid)</li>
</ul>';
...
<p>This formula may be used to create a Margoyle. The minimum level becomes 17, the cost becomes 10,000 gp, and the Stoneworking DC becomes 20</p>';
        $helper->saveFormula($formula, ['Elemental', 'Earth', 'Enchantment'], [
            'skills'    => ['Crafting' => ['dc' => 15, 'meta' => 'Stoneworking']],
...
<ul>
    ...
    <li>Assembling the body requires a DC 20 Craft (Armorsmithing) check or a DC 20 Craft (Weaponsmithing) check.</li>
</ul>";
        $formula->description = '<p>You create a Iron Golem.</p>';
        $helper->saveFormula($formula, ['Construct'], [
...
<ul>
    ...
    <li>Assembling the body requires a DC 17 Craft (Sculpting) check or a DC 17 Craft (Stonemasonry) check.</li>
</ul>";
...
<p>Effigy creatures do only what they are instructed to do by their creators. They draw no conclusions of their own and follow orders explicitly with no regard for their own safety. The commands given them must be simple, such as 'Remain here and attack any elves who enter this room,' or 'Follow and defend against any who attack my party.'</p>
<p>An Effigy's creator can command it if the effigy is within 60 feet and can see and hear him with an Action. If un-commanded, an effigy usually follows its last instruction to itself if attacked. An Effigy's creator can order the creature to obey the orders of another individual with an Action (who can in turn order effigy to obey someone else), but the Effigy's creator can always resume control over his creation by commanding the effigy to obey him alone with an Action.</p>
<p>Damaged effigies can be repaired by means of any repair spell or by manual repair in an appropriate workshop or laboratory. The individual performing the repairs must have the Craft Construct feat or 10 ranks in Knowledge (architecture and engineering), and must spend 50 gp and 1 hour per hit point restored.</p>";
        $helper->saveFormula($formula, ['Construct'], [
            'skills' => ['Crafting' => ['dc' => 10, 'meta' => 'Metalworking or Clockwork']],
            'feats'  => ['Craft Effigy'],
        ]);
    }
}
