...
<p>If you do not have the ability to cast spells, you now can cast spells from scrolls (As if you had the Activate Magic Item Feat).</p>";
        $helper->addTypesToFeat($feat, ['Downtime', 'Manipulate', 'Skill', 'Item Creation']);
...
<p>A Dispel Magic spell of any level that targets a temporary Scroll turns them into blank sheets of paper.</p>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Manipulate', 'Item Creation']);
...
<table>
    ...
    <tbody>
        ...
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Acid', 'Bomb', 'Drug', 'Elixir', 'Oil', 'Poison', 'Potion']);
...
<p>When you select this feat, you gain the formulas for four common armor or weapons of level 2 or lower.</p>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Psionic', 'Armor']);
        $feat->skills()->save(app()->skills['Crafting'], ['dc' => 2]);
...
<p>When you select this feat, you gain the formulas for four common wands, and/or scepters of level 2 or lower.</p>
...
<p>In general, the base price to craft a Wand is (Caster Level X Spell Level X 10 X number of charges gp. Assume Cantrips have a Spell Level of 1/2). Staffs can vary since they can have multiple spells but typically uses the same cost formula as the wand for each spell, then multiply it all by 10.</p>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Wand']);
        $feat->skills()->save(app()->skills['Crafting'], ['dc' => 2]);
...
<p>When you select this feat, you gain the formulas for four common rings of level 2 or lower.</p>
...
<p>If you inscribe a Rune onto a piece of Armor or a Weapon, treat it as if it was created using the Craft Armament feat. Inscribing a Rune into an area requires X10 the normal amount of gp.</p>';
        $helper->addTypesToFeat($feat, ['Divine', 'Downtime', 'Item Creation']);
...
<p>If you do not have the ability to trigger magic items (typically with the Activate Magic Item feat), then you can now activate Tattoos.</p>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Psionic', 'Attunement']);
...
<p>Crafting a cognizance crystal causes you 1/200 of its base price in CON damage and use up raw materials costing one-half its base price.</p>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Psionic']);
...
<ul>
    <li>When you select this feat, you gain the formulas for four common items of level 2 or lower.</li>
    <li>If you also have the Craft Armament feat, you can also create Constructs.</li>
    <li>If you also have the Expert Conjurer feat, you can also create Portals.</li>
    <li>If you also have the Craft Implement feat, you can also create Rods, Staffs</li>
    <li>If you also have the Imprint Stone feat, then you can also create Psionic Seals (Psionic Runes).</li>
    <li>If you also have the Craft Cognizance Crystal feat and Craft Dorje feat, then you can create Psicrowns.</li>
</ul>';
        $helper->addTypesToFeat($feat, ['Downtime', 'Item Creation', 'Psionic', 'Construct', 'Potion', 'Scroll', 'Portal']);
        ...
        $feat->description = '<p>You gain 4 Spell Points.</p>
<p>Spend 2 Spell Point and you gain Advantage on your Ranged Spell attack roll.</p>';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);

        $feat              = new Feat;
        $feat->name        = 'Improved Aim Spell';
        $feat->trigger     = 'You are casting a spell that requires a Ranged Spell attack roll';
        $feat->description = '<p>You gain 5 Spell Points.</p>
<p>Spend 5 Spell Points and your Ranged Spell attack automatically hits.</p>';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
        ...
        $feat->description = "<p>You add a Somatic Casting action to the casting of the triggering spell and attempt a Stealth check against one or more observers' Perception DCs. If you beat an observer's Perception DC, that observer doesn't notice you are casting a spell; if the spell has a Verbal Casting action, you must also roll a Deception check against the observer's Perception DC, or the observer notices the spell. This conceals only the spell's spellcasting actions, not any of the spell's physical effects, so an observer might still see a ray streak out from you or see you disappear.</p>";
        $helper->addTypesToFeat($feat, ['Metamagic']);
        ...
        $feat->description = "<ul>
    <li>Spend 6 Spell Points. You cast a spell and set a delay of 1 to 5 rounds before it takes effect. The delay time cannot be changed once set; the spell activates at the beginning of your turn you designate. Only area, personal, and touch spells can be affected by this feat.</li>
    <li>Any decisions you would make about the spell (including attack rolls, designating targets, or determining or shaping an area) are decided when the spell is cast, with any of its effects (including damage and Saves) decided when the spell is triggers. If conditions change during the delay period in ways that would make the spell impossible to cast (the target you designate moves beyond the spell's range, for example), the spell fails.</li>
...
<p>Like a Silent, Stilled Spell, a Disguised Spell can't be identified through Arcana. Your performance is obvious to everyone in the vicinity, but the fact that you are casting a spell isn't, unless the spell visibly emanates from you or observers have some other means of determining its source, they don't know where the effect came from.</p>";
        $helper->addTypesToFeat($feat, ['Metamagic', 'Extra Action', 'Spell Pool']);
...
<p>You may take this feat multiple times. Each time choose a different type of energy.</p>';
        $feat->action_type = 'Free';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Evocation', 'Spell Pool']);
    ...
    <li>If you spend 5 Spell Points instead, the casting of the spell no longer requires an Action for the Material Casting (as long as the Materials total cost is less than 50 gp).</li>
</ul>';
        $feat->action_type = 'Free';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
    ...
    <li>Spend 5 Spell Points and a spell that requires Concentration does not require Concentration. This allows you to cast another spell that Requires Concentration.</li>
</ul>';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
    ...
    <li>Spend 2 Spell Points. Triple the range of the casting spell.</li>
        ...
        $feat->description = '<ul>
    <li>You gain 2 Spell Points</li>
    <li>Spend 4 Spell Points to modify its range to 30 feet. If the touch spell normally requires an attack roll or a touch attack roll, it now requires a ranged attack roll or a ranged touch attack roll, as appropriate.</li>
</ul>';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
...
<p>You cannot repeat a spell with a range of Touch.</p>';
    ...
    <li>If you spend 5 Spell Points instead, the casting of the spell no longer requires an Action for the Verbal Casting.</li>
</ul>';
        $feat->action_type = 'Free';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
    ...
    <li>If you spend 5 Spell Points instead, the casting of the spell no longer requires an Action for the Somatic Casting.</li>
</ul>';
        $feat->action_type = 'Free';
        $helper->addTypesToFeat($feat, ['Metamagic', 'Spell Pool']);
...
<p>You may take this feat multiple times.</p>';
        $helper->addTypesToFeat($feat, ['Tattoo', 'Attunement']);
